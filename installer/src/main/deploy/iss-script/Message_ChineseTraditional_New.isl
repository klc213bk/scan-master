...
DownloadingLabel=附加檔案下載中...
ButtonStopDownload=停止下載 (&S)
StopDownload=您確定要停止下載嗎?
ErrorDownloadAborted=下載中止
ErrorDownloadFailed=下載失敗: %1 %2
ErrorDownloadSizeFailed=無法取得檔案大小: %1 %2
ErrorFileHash1=檔案雜湊值驗證失敗: %1
ErrorFileHash2=檔案雜湊值不正確: 應為 %1, 現為 %2
ErrorProgress=進度不正確: %1 / %2
ErrorFileSize=檔案大小不正確: 應為 %1, 現為 %2
; <--
...
; VC -->
PrepareToInstallNeedsRestart=安裝程式必須重新啟動您的電腦。在重新啟動電腦之後，請再重新執行這個安裝程式以完成 [name] 的安裝。%n%n您想要現在重新啟動電腦嗎?
; <--
...
FileExists2=檔案已經存在。
FileExistsOverwriteExisting=覆寫目前已存在的檔案 (&O)
FileExistsKeepExisting=保留目前已存在的檔案 (&K)
FileExistsOverwriteOrKeepAll=其他檔案已經存在時也比照處理 (&D)
ExistingFileNewerSelectAction=選取動作
ExistingFileNewer2=已存在的檔案版本比您目前要安裝的版本還要新。
ExistingFileNewerOverwriteExisting=覆寫目前已存在的檔案 (&O)
ExistingFileNewerKeepExisting=保留目前已存在的檔案 (建議選項) (&K)
ExistingFileNewerOverwriteOrKeepAll=其他檔案已經存在時也比照處理 (&D)
; <--
